...
5.上面的是点对点模型(1个消费者对应一个生产者),下面是发布/订阅模型
 ...
 2).可视化插件：https://github.com/apache/rocketmq-externals.git, bin目录下执行git: mvn clean package -Dmaven.test.skip=true,首先修改application.properties中nameserver地址rocketmq.config.namesrvAddr=192.168.0.101:9876,默认端口localhost:8080
